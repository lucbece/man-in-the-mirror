...
# The only thing this image needs beyond Node: an ssh client. The zomboid
# character asks the operator that lives on the game server through a key
# pinned to one command, and without a client there is nothing to ask with.
# `--no-install-recommends` keeps it to the client rather than a server.
RUN apt-get update \
