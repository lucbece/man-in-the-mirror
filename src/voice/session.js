...
import { endsWithQuestion, isReturnQuestion } from '../agent/return-question.js';

// Re-exported: this is where the tests and the rest of voice/ have always
// imported these from, and agent/index.js reaches for the same predicates to
// decide whether to hold a sentence back before it's spoken. See
// agent/return-question.js for the shared implementation.
export { endsWithQuestion, isReturnQuestion };
...
export class VoiceSession extends EventEmitter {
   ...
   * a log line.
   *
   * Both windows are opt-in, behind `strictWake`: under it neither one is
   * armed, and the name — plus the held-question replay and the mode/hush
   * commands — is the only way in. That takes `looksLikeFollowUp` out of the
   * loop too, since it is only ever consulted through the window this sets.
   */
  expectReply(userId, spoken) {
    if (!userId || !String(spoken ?? '').trim() || config.get('strictWake')) {
      this.awaitingReply = null;
      return false;
    }
