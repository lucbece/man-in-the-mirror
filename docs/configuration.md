...
| `characters` | — | — | The bot's other characters, as a JSON array. Deliberately empty in the repository: a character names a Discord role, a text channel and a job, which are facts about one group of people. Fields: `name`, `displayName`, `spoken` (the phrasings that ask for it), `enterRole` and `actRole` (Discord role names), `voice`, `detailChannel`, `tools` (which families it keeps), `entering`, `leaving`, `prompt` |
...
| `zomboidSsh` | — | — | `user@host` of the game VM, where the operator that reads its logs runs. Two keys in the volume, mode 0600: `data/zomboid-key` to look and `data/zomboid-key-act` to change something, each pinned in the VM's `authorized_keys` to one script with `command=`. A forced command ignores what the client asks for, so which key is used *is* the permission; without the second key the character can only look. Blank turns `zomboid_ask` off |
...
| `zomboid_ask` | Asks the operator running on the game VM — it reads the logs, the configuration and the state of the machine and answers with what it found. Read-only unless `act` is set, which needs the character's `actRole`; the long half of the answer is written to the character's channel and only one line is spoken. Up to 150 s |
