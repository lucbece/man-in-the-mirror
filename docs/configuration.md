...
| `zomboidSsh` | — | — | `user@host` of the game VM, where the operator that reads its logs runs. The key is `data/zomboid-key` (mode 0600, in the volume) and its entry in the VM's `authorized_keys` pins it to one script with `command=`, so it is a door onto one question rather than an account. Blank turns `zomboid_ask` off |
