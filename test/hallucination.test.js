...
import { looksHallucinated, echoesPrompt, namedByNoise, namesNoise } from '../src/agent/stt.js';
...
describe('namedByNoise: the name alone, in a clip that was never loud', () => {
  const quiet = { activeRatio: 0.02 };
  const spoken = { activeRatio: 0.2 };
  ...
  test('the same word over a clip somebody actually spoke into is a call, even a quiet one', () => {
    assert.equal(namedByNoise('espejo', 'mirror, espejo', spoken), false);
    assert.equal(namedByNoise('espejo', 'mirror, espejo', { activeRatio: 0.12 }), false);
  });

  test('only the GPT-4o transcribers get the guard at all', () => {
    assert.equal(namesNoise('gpt-4o-transcribe'), true);
    assert.equal(namesNoise('gpt-4o-mini-transcribe'), true);
    assert.equal(namesNoise('whisper-1'), false);
  });
